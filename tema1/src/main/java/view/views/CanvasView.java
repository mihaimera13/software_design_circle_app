...
import presenter.CanvasGeometryPresenter;
import presenter.CanvasUserPresenter;
import view.resources.DrawingCanvas;
import view.resources.ICanvasView;
...
public class CanvasView extends JFrame implements ActionListener, ChangeListener, ICanvasView {
    ...
    private CanvasGeometryPresenter canvasGeometryPresenter;
    private CanvasUserPresenter canvasUserPresenter;
    
    public CanvasView(){
        canvasGeometryPresenter = new CanvasGeometryPresenter(this);
        canvasUserPresenter = new CanvasUserPresenter(this);
        this.setMainPanel();
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(command.equals("CHANGE COLOR")){
            ...
            canvasGeometryPresenter.changeColor(source);
        }
        else if(command.contains("CHANGE DASHING")){
            char type = command.charAt(command.length()-1);
            switch (type) {
                case '1' -> canvasGeometryPresenter.changeStroke();
                case '2' -> canvasGeometryPresenter.changePattern(new float[]{6.0F, 3.0F});
                case '3' -> canvasGeometryPresenter.changePattern(new float[]{1.0F, 1.0F});
            }
        }
        else if(command.equals("LOGIN")){
            canvasUserPresenter.openLogin();
        }
        else if(command.equals("ACCOUNT")){
            canvasUserPresenter.openNewAccount();
        }
        else if(command.equals("CIRCLE")){
            canvasGeometryPresenter.displayCircleInfo();
        }
        else if(command.equals("CIRCUMSCRIBED")){
            canvasGeometryPresenter.drawCircumscribed();
        }else if(command.equals("INSCRIBED")){
            canvasGeometryPresenter.drawInscribed();
        }
    ...
    public void stateChanged(ChangeEvent e) {
        canvasGeometryPresenter.changeStroke();
    }
