...
public class Polygon implements GeometryObject {
    ...
    public Point getIncenter(){
        ...
        if(this.vertices.size()==3){
            ArrayList<Line> lines = this.transformToLines();
            float x = (float) ((this.vertices.get(0).getX()*lines.get(2).computeLength() +
                                this.vertices.get(1).getX()*lines.get(0).computeLength() +
                                this.vertices.get(2).getX()*lines.get(1).computeLength())/
                                (lines.get(0).computeLength() + lines.get(1).computeLength() + lines.get(2).computeLength()));
            float y = (float) ((this.vertices.get(0).getY()*lines.get(2).computeLength() +
                    this.vertices.get(1).getY()*lines.get(0).computeLength() +
                    this.vertices.get(2).getY()*lines.get(1).computeLength())/
                    (lines.get(0).computeLength() + lines.get(1).computeLength() + lines.get(2).computeLength()));
